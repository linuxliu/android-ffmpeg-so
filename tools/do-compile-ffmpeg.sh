...
if [ "$FF_ARCH" = "armv7a" ]; then
    ...
    FF_BUILD_NAME_RTMPDUMP=rtmpdump-x86
    ...
    FF_EXTRA_LDFLAGS="$FF_EXTRA_LDFLAGS -fPIC"
    ...
    FF_BUILD_NAME_RTMPDUMP=rtmpdump-arm64
...
FF_DEP_RTMPDUMP_INC=$FF_BUILD_ROOT/build/$FF_BUILD_NAME_RTMPDUMP/output/include
FF_DEP_RTMPDUMP_LIB=$FF_BUILD_ROOT/build/$FF_BUILD_NAME_RTMPDUMP/output/lib
...
if [ -f "${FF_DEP_LIBFDK_AAC_LIB}/libfdk-aac.a" ]; then
    ...
    FF_CFG_FLAGS="$FF_CFG_FLAGS --enable-libfdk-aac --enable-decoder=libfdk_aac --enable-encoder=libfdk_aac"
    FF_EXTRA_LDFLAGS="$FF_EXTRA_LDFLAGS -lm" 

    FF_CFLAGS="$FF_CFLAGS -I${FF_DEP_LIBFDK_AAC_INC}"
    FF_DEP_LIBS="$FF_DEP_LIBS -L${FF_DEP_LIBFDK_AAC_LIB} -lfdk-aac"
fi
# with libx264
